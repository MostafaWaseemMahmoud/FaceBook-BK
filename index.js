...
app.post("/addpost/:userId", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    const newPost = { imageUrl, title, post, comments, likes }; //? Create a new post object
...
app.post("/addFriend", async (req, res) => {
  ...
  try {
    const friend = await User.findById(friendId); // Find the friend by ID and select relevant fields
    const user = await User.findById(userId); // Find the user by ID and select relevant fields
